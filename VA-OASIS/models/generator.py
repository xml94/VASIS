...
class ResnetBlock_with_SPADE(nn.Module):
    def __init__(self, fin, fout, opt, height, width):
        ...
        self.conv_0 = sp_norm(nn.Conv2d(fin, fmiddle, kernel_size=3, padding=0))
        self.conv_1 = sp_norm(nn.Conv2d(fmiddle, fout, kernel_size=3, padding=0))
    ...
    def forward(self, x, seg):
        if self.learned_shortcut:
            ...
            x_s = x
        dx = self.pad(self.conv_0(self.activ(self.norm_0(x, seg))))
        dx = self.pad(self.conv_1(self.activ(self.norm_1(dx, seg))))
        out = x_s + dx
        return out
